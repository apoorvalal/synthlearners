...
class MatrixCompletionEstimator:
    ...
    def update_u(self,M, mask, L, v):
        ...
        """
        M, mask, L, v = map(np.asarray, (M, mask, L, v))  # Convert inputs to numpy arrays
    ...
    def update_v(self,M, mask, L, u):
        ...
        """
        M, mask, L, u = map(np.asarray, (M, mask, L, u))  # Convert inputs to numpy arrays
        
        b = L + u[:, np.newaxis] - M  # Compute the difference matrix
        b_masked = b * mask  # Apply element-wise mask
        mask_counts = np.count_nonzero(mask, axis=0)  # Count non-zero elements per column
    ...
    def compute_objval(self, M, mask, L, u, v, sum_sing_vals, lambda_L):
        ...
        """
        M, mask, L, u, v = map(np.asarray, (M, mask, L, u, v))  # Convert inputs to numpy arrays
